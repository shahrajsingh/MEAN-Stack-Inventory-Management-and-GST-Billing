...
export class AuthService {
  ...
  getuserdata(): Observable<{ name: string; email: string }> {
    return this.http.get<{ name: string; email: string }>(
      BACKEND_URL + 'getuser/' + this.userId
    );
  }
  autoAuthUser() {
  ...
  login(email: string, pass: string) {
    const authData = { email: email, password: pass };
    this.http
      .post<{
        token: string;
        expiresIn: number;
  ...
  updateinfo(name: string, email: string, ques: string, ans: string) {
    ...
    this.http
      .put<{ message: string; result: number }>(BACKEND_URL + this.userId, body)
      .subscribe((res) => {
        if (res.result != 0) {
          this.router.navigate(['/app']);
